                ...
                <div class="p-6 text-gray-900">
                    <div class="container mx-auto p-4 w-1/2">
                        <div class="bg-white rounded-lg shadow-lg p-6">
                            ...
                            <h1 class="text-2xl font-semibold my-4">Edit Score</h1>
                            <form action="{{ route('kinerja.update', $score->id) }}" method="POST">
                                @csrf
                                @method('PUT')
                                <div class="mb-4">
                                    ...
                                    <select name="candidate_id" id="candidate_id" class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-600">
                                        @foreach($candidates as $candidate)
                                            <option value="{{ $candidate->id }}" {{ $candidate->id == $score->candidate_id ? 'selected' : '' }}>{{ $candidate->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="mb-4">
                                    ...
                                    <select name="criteria_id" id="criteria_id" class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-600">
                                        @foreach($criterias as $criteria)
                                            <option value="{{ $criteria->id }}" {{ $criteria->id == $score->criteria_id ? 'selected' : '' }}>{{ $criteria->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="mb-4">
                                    <label for="value" class="block text-gray-700">Value</label>
                                    <input type="number" name="value" id="value" class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-600" step="0.01" min="0" max="100" value="{{ $score->value }}">
                                </div>
